...
<body>
	...
	try {
		...
		ResultSet rs2 = st.executeQuery("select * from user inner join card where card.email='" + email
		+ "' and user.email='" + email + "' and card.status='bill'");
		while (rs2.next()) {
	%>
	...
	<table id="customers">
		...
		<%
		ResultSet rs3 = st
				.executeQuery("select * from card inner join product where card.product_id=product.id and card.email='" + email
				+ "' and card.status='bill'");
		while (rs3.next()) {
			sno = sno + 1;
		%>
		<tr>
