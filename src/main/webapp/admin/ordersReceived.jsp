...
<body>
	...
	<table id="customers">
		...
		try {
			...
			"select * from card inner join product where card.product_id=product.id and card.orderdate is not NULL and card.status='processing'");
			while (rs.next()) {
		%>
		...
		<tr>
			...
			<td><a
				href="cancleOrderAction.jsp?id=<%=rs.getString(2)%>&email=<%=rs.getString(1)%>">Cancel
